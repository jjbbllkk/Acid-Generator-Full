...
struct AcidSeqWidget : ModuleWidget {
    ...
    AcidSeqWidget(AcidSeq* module) {
        ...
        const float LEFT_SECTION = 10.f;   // Center of left knob column
        const float RIGHT_SECTION = 26.f;  // Center of right knob column
        ...
        // === Piano Roll Display ===
        {
            pianoRoll = new PianoRollDisplay();
            pianoRoll->box.pos = mm2px(Vec(DISPLAY_X, 12));
            pianoRoll->box.size = mm2px(Vec(DISPLAY_WIDTH, 36));
            pianoRoll->module = module;
        ...
        float indicatorSpacing = 3.f;

        // Octave row
        {
            octaveRow = new StepIndicatorDisplay();
            octaveRow->box.pos = mm2px(Vec(DISPLAY_X, indicatorY));
            octaveRow->box.size = mm2px(Vec(DISPLAY_WIDTH, indicatorHeight));
            octaveRow->rowType = StepIndicatorDisplay::RowType::OCTAVE;
            octaveRow->module = module;
        ...
        // Slide row
        {
            slideRow = new StepIndicatorDisplay();
            slideRow->box.pos = mm2px(Vec(DISPLAY_X, indicatorY + indicatorSpacing));
            slideRow->box.size = mm2px(Vec(DISPLAY_WIDTH, indicatorHeight));
            slideRow->rowType = StepIndicatorDisplay::RowType::SLIDE;
            slideRow->module = module;
        ...
        // Accent row
        {
            accentRow = new StepIndicatorDisplay();
            accentRow->box.pos = mm2px(Vec(DISPLAY_X, indicatorY + indicatorSpacing * 2));
            accentRow->box.size = mm2px(Vec(DISPLAY_WIDTH, indicatorHeight));
            accentRow->rowType = StepIndicatorDisplay::RowType::ACCENT;
            accentRow->module = module;
        ...
        // Step numbers
        {
            stepNumbers = new StepNumberDisplay();
            stepNumbers->box.pos = mm2px(Vec(DISPLAY_X, indicatorY + indicatorSpacing * 3));
            stepNumbers->box.size = mm2px(Vec(DISPLAY_WIDTH, 3.f));
            stepNumbers->module = module;
            stepNumbers->viewOffsetPtr = &pianoRoll->viewOffset;  // Sync scrolling
            if (module) {
                stepNumbers->currentStepPtr = &module->currentStep;
                stepNumbers->patternLengthPtr = &module->cachedPatternLength;
            }
            addChild(stepNumbers);
        }
